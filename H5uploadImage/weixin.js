...
(function(){	
	...
	/*	
		已添加公众号：
		射雕 32,wx2bbca7621450dde7
		笑傲 23,wx40d5f7c12ebd4767
		完美国际 3,wxdb47b133c958b4d8
	*/
	var debug = /wx_debug/.test(location.search)
	var aid = getScriptParams().aid || 3
	var apiurl = 'http://passport.wanmei.com/api/wxjs.do?method=auth&jsonpCallback=wx.auth_jsonp&url='+encodeURIComponent(location.href.split('#')[0])
	
	if(aid){	
		apiurl += '&aid=' + aid
	}

	wx.auth_jsonp = function(json){	
		if(json.code === 0){
			var data = json.data
			wx.config({
				debug: debug,
